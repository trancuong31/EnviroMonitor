...
const LocationListItem = ({ location, locationId, temperature, humidity, sensorType = 'ROOM', chartData = [], lastUpdate, lastUpdateISO, onClick, tempMin, tempMax, humMin, humMax }) => {
    ...
    return (
        <div
            onClick={onClick}
            className={`group bg-surface rounded-2xl px-8 py-2 border-1 shadow-sm grid grid-cols-1 md:grid-cols-[auto_1fr_auto_auto] gap-6 md:gap-8 items-center transition-all duration-300 hover:shadow-md hover:translate-x-2 cursor-pointer ${
                isOffline
