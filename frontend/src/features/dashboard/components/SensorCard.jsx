...
const LocationCard = ({ location, locationId, temperature, humidity, sensorType = 'ROOM', lastUpdate, lastUpdateISO, status = 'Normal', onClick }) => {
    ...
    return (
        <div
            onClick={onClick}
            className={`group relative bg-surface rounded-[20px] p-7 border-2 overflow-hidden transition-all duration-400 cursor-pointer hover:-translate-y-2 hover:shadow-[0_10px_40px_rgba(0,0,0,0.12)] ${
                isOffline
                    ? 'border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.15)] hover:border-red-500/80'
                    : hasWarning
        ...
        >
            ...
            <div className="flex justify-between text-white items-start mb-5 relative z-10">
                <div className="flex items-center text-white gap-2 ">
                    <div className={`w-10 h-10 text-white rounded-xl flex items-center justify-center text-lg border animate-pulse-icon ${
                        isOffline 
                    ...
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg border animate-pulse-icon ${
                        isOffline 
                            ? 'bg-red-500/10 text-red-500 border-red-500/30'
                            : hasWarning && humWarning
                                ? 'bg-warning/10 text-warning border-warning/30'
                ...
                {(hasWarning || isOffline) && (
                    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold border ${
                        ...
                        <span>{isOffline ? 'NG' : t('dashboard.warning', 'Cảnh báo')}</span>
                    </div>
                )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-5 relative z-10 mb-6">
                ...
                <div>
                    ...
                    <div className="flex items-baseline gap-1.5">
                        <span className={`text-[2.2rem] font-bold font-mono tracking-tight leading-none bg-clip-text text-transparent ${
                            isOffline
                                ? 'bg-red-500'
                                : tempWarning
            ...
            <div className="flex justify-between pt-4 border-t border-border text-xs text-text-muted relative z-10">
                ...
                <div className={`font-medium ${isOffline ? 'text-red-500' : (hasWarning ? 'text-warning' : '')}`}>
                    {displayStatus}
                </div>
            </div>
        </div>
