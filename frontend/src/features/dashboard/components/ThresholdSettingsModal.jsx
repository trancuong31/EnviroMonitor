...
const ThresholdSettingsModal = ({ isOpen, onClose, onRefresh }) => {
  ...
  const handleSave = async () => {
    ...
    const originalValues =
      activeTab === 'WHC' ? WHC : activeTab === 'WHN' ? WHN : activeTab === 'PL' ? PL : ng;
    const newValues = formValues[activeTab];
    const isTabSame = Object.keys(originalValues).every(
      (key) => originalValues[key] === newValues[key]
    );
  ...
  const tempMaxRange =
    activeTab === 'WHC' ? 20 : activeTab === 'WHN' ? 50 : activeTab === 'PL' ? 50 : 50;
          ...
          <div className="space-y-5">
            ...
            <div className="relative h-3 bg-gradient-to-r from-blue-400 via-green-400 to-red-400 rounded-full overflow-hidden">
              ...
              <div
                ...
                style={{
                  left: 0,
                  width: `${((currentValues.tempMin - (activeTab === 'WHC' ? -10 : 0)) / tempMaxRange) * 100}%`,
                }}
              />
              <div
                className="absolute top-0 h-full bg-surface/80 rounded-r-full"
                style={{
                  left: `${((currentValues.tempMax - (activeTab === 'WHC' ? -10 : 0)) / tempMaxRange) * 100}%`,
                  right: 0,
                }}
              />
              {/* Markers */}
              <div
                ...
                style={{
                  left: `${((currentValues.tempMin - (activeTab === 'WHC' ? -10 : 0)) / tempMaxRange) * 100}%`,
                }}
              />
              <div
                className="absolute top-0 w-0.5 h-full bg-temp shadow-lg"
