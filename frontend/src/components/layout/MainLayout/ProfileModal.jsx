...
const ProfileModal = ({ isOpen, onClose }) => {
  ...
  const handleSavePassword = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || t('profile.updatePasswordError', 'Failed to update password')
      );
    } finally {
      setSaving(false);
    }
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-4 mb-1">
                ...
                <div className="min-w-0">
                  <h3 className="text-lg font-bold truncate" title={userData.fullname}>
                    {userData.fullname}
                  </h3>
                  <p className="text-sm text-text-muted truncate" title={userData.userid}>
                    {userData.userid}
                  </p>
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <div className="flex items-center gap-2.5 p-2.5 bg-surface-alt/50 rounded-xl border border-border">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] font-semibold uppercase truncate capitalize">
                        {userData.role || 'User'}
                      </p>
                    </div>
                  </div>
