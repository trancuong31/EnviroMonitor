  ...
  return (
    <div className="flex flex-col min-h-screen">
      <header className="sticky top-0 z-50 flex items-center justify-between px-6 md:px-8 py-1 bg-surface/80 backdrop-blur-xl border-b border-border shadow-sm transition-colors duration-300">
        <Link
          to="/"
          className="text-xl font-bold font-mono bg-gradient-to-r from-primary to-primary-dark bg-clip-text text-transparent hover:opacity-80 transition-opacity"
        >
          {t('dashboard.title')}
        </Link>
              ...
              <div className="hidden sm:flex items-center gap-2 text-sm text-text-muted">
                {user?.role === 'Admin' && (
                  <>
                    <Link
                      to="/admin/users"
                      className="relative group flex items-center h-full pt-1 pb-1"
                    >
                      <Button
                        variant="ghost"
                        size="small"
                      ...
                      <span
                        className={`absolute bottom-0 left-2 right-2 h-[2.5px] bg-primary rounded-t-full transition-transform duration-300 origin-center ${
                          location.pathname.startsWith('/admin/users')
                            ? 'scale-x-100'
                    ...
                    >
                      <Button
                        ...
                        size="small"
                        className={`text-primary transition-colors hover:bg-transparent ${
                          location.pathname.startsWith('/admin/translations')
                            ? 'bg-primary/10 font-semibold'
                            : ''
                        }`}
                      >
                        {t('admin.translationManagement', 'Languages')}
                      </Button>
